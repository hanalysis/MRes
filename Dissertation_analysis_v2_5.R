...
# Replacing missing / incorrect with 0 
...
SummaryStats <- midus_clean %>%
  summarise("Age; mean" = mean(age),
            ...
            "CRP, mean" = mean(CRP),
            SDCRP = sd(CRP),
            "Word recall, mean" = mean(WordRecall_percent),
            SDWordRecall = sd(WordRecall_percent))
...
Cog_CTQ_Recall <- lm(WordRecall_percent ~ CTQ_Scored, 
                     data = midus_clean)

Cog_CTQ_Sub_Recall <- lm(WordRecall_percent ~ CTQ_EmotionalAbuse + 
                           CTQ_EmotionalNeglect + 
                           CTQ_PhysicalAbuse+ 
                           CTQ_PhysicalNeglect + 
                           CTQ_SexualAbuse, 
...
Cog_MASQ_Recall <- lm(WordRecall_percent ~ MASQ_AD, 
                     data = midus_clean)
...
Cog_MASQDD_Recall <- lm(WordRecall_percent ~ MASQ_GenDistressDepress, 
                      data = midus_clean)
...
anova_WordRecall <- aov_4(WordRecall_percent ~ Group + (1 | num), data = MIDUS_grouped)

summary(anova_WordRecall)
### F = 2.1, p-value = 0.1023
